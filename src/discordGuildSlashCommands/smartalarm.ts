...
export default {
	...
	async execute(dm: DiscordManager, interaction: discordjs.ChatInputCommandInteraction): Promise<boolean> {
		const fn = '[SlashCommand: smartalarm]';
		const logParam = {
...
async function executeEdit(dm: DiscordManager, interaction: discordjs.ChatInputCommandInteraction): Promise<boolean> {
	const fn = '[SlashCommand: smartalarm: edit]';
	...
	if (serverId === null) {
		...
		log.warn(`${fn} ${id} ${lm.getIntl(config.general.language, 'errorDescSmartalarmDoesNotExist', parameters)}`,
			logParam);
		return false;
	}
	...
	log.info(`${fn} ${id} ${lm.getIntl(config.general.language, 'slashCommandSuccessDescSmartalarmEdit', parameters)}`,
		logParam);
