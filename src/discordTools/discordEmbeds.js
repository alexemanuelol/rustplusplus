...
module.exports = {
    ...
    getServerEmbed: async function (guildId, serverId) {
        ...
        if (credentials.hasOwnProperty(server.steamId)) {
            hoster = await discordTools.getMember(Client.client, guildId, credentials[server.steamId].discord_user_id);
    ...
    getStorageMonitorNotFoundEmbed: async function (guildId, serverId, entityId) {
        ...
        const credentials = Credentials.readCredentialsFile();
        const user = await discordTools.getMember(Client.client, guildId, credentials[server.steamId].discord_user_id);
        const grid = entity.location !== null ? ` (${entity.location})` : '';
    ...
    getCredentialsShowEmbed: async function (guildId) {
        ...
        for (const credential in credentials) {
            ...
            const user = await discordTools.getMember(Client.client, guildId, credentials[credential].discord_user_id);
