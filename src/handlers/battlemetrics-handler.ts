...
export async function battlemetricsHandler(firstTime: boolean = false) {
    ...
    for (const guildItem of client.guilds.cache) {
        ...
        if (!firstTime) await handleBattlemetricsChanges(guildId);
        ...
        if (condition) {
            await discordMessages.sendUpdateBattlemetricsOnlinePlayersInformationMessage(rustplus, bmId as string);
        }
        else {
            if (instance.informationChannelMessageIds.battlemetricsPlayers !== null) {
                await discordTools.deleteMessage(guildId, instance.channelIds.information as string,
        ...
        for (const [trackerId, content] of Object.entries(instance.trackers)) {
            ...
            if (firstTime || searchSteamProfiles) {
                for (const player of content.players) {
                    ...
                    if (calledSteamProfiles.hasOwnProperty(player.steamId)) {
                        name = (calledSteamProfiles as any)[player.steamId];
                    }
                    else {
                        name = await requestSteamProfileName(player.steamId);
                        (calledSteamProfiles as any)[player.steamId] = name;
                    }
                    ...
                    if (player.name !== name) {
                        await trackerNewNameDetected(guildId, trackerId, battlemetricsId, player.name as string, name);

                        const newPlayerId = Object.keys(bmInstance.players).find(e =>
                            bmInstance.players[e]['name'] === name);
            ...
            for (const player of bmInstance.nameChangedPlayers.filter((e: any) => trackerPlayerIds.includes(e.id))) {
                for (const playerT of content.players) {
                    if (playerT.playerId !== player.id) continue;

                    await trackerNewNameDetected(guildId, trackerId, battlemetricsId, player.from, player.to);
                }
            }
            ...
            for (const playerId of trackerPlayerIds.filter(e => bmInstance.newPlayers.includes(e))) {
                for (const player of content.players) {
                    ...
                    const str = lm.getIntl(language, 'playerJustConnectedTracker', {
                        name: player.name,
            ...
            for (const playerId of trackerPlayerIds.filter(e => bmInstance.logoutPlayers.includes(e))) {
                for (const player of content.players) {
                    ...
                    const str = lm.getIntl(language, 'playerJustDisconnectedTracker', {
                        name: player.name,
                        tracker: content.name
                    });
...
async function handleBattlemetricsChanges(guildId: string) {
    const instance = guildInstance.readGuildInstanceFile(guildId);
    const settings = instance.generalSettings;
    const language = settings.language;

    const activeServer = instance.activeServer as string;
    ...
    for (const battlemetricsId of battlemetricsIds) {
        ...
        if (settings.battlemetricsServerNameChanges && bmInstance.serverEvaluation.hasOwnProperty('server_name')) {
            ...
            const title = lm.getIntl(language, 'battlemetricsServerNameChanged');
            const description = `__**${lm.getIntl(language, 'old')}:**__ ${oldName}\n` +
                `__**${lm.getIntl(language, 'new')}:**__ ${newName}`;
        ...
        if (settings.battlemetricsGlobalNameChanges && bmInstance.nameChangedPlayers.length !== 0) {
            const title = lm.getIntl(language, 'battlemetricsPlayersNameChanged');

            const oldNameFieldName = lm.getIntl(language, 'old');
            const playerIdFieldName = lm.getIntl(language, 'playerId');
            const newNameFieldName = lm.getIntl(language, 'new');
            ...
            let description = '';
            if (isEmbedFull) {
                description = lm.getIntl(language, 'andMorePlayers', {
            ...
            for (let i = 0; i < (fieldIndex + 1); i++) {
                fields.push({
                    ...
                    value: oldName[i] !== '' ? oldName[i] : lm.getIntl(language, 'empty'),
                    inline: true
                });
                fields.push({
                ...
                fields.push({
                    ...
                    value: newName[i] !== '' ? newName[i] : lm.getIntl(language, 'empty'),
                    inline: true
                });
            }
        ...
        if (settings.battlemetricsGlobalLogin &&
            ...
            const title = lm.getIntl(language, 'battlemetricsPlayersLogin');
            ...
            for (const playerId of playerIds) {
                playerCounter += 1;
                const name = bmInstance.players[playerId as string]['name'].replace('[', '(').replace(']', ')');
        ...
        if (settings.battlemetricsGlobalLogout && bmInstance.logoutPlayers.length !== 0) {
            const title = lm.getIntl(language, 'battlemetricsPlayersLogout');
...
async function trackerNewNameDetected(guildId: string, trackerId: string, battlemetricsId: string,
    oldName: string, newName: string) {
    const instance = guildInstance.readGuildInstanceFile(guildId);
    const language = instance.generalSettings.language;
    const trackerName = instance.trackers[trackerId].name;

    const title = lm.getIntl(language, 'battlemetricsTrackerPlayerNameChanged');
    const description = `__**${lm.getIntl(language, 'tracker')}:**__ ${trackerName}\n\n` +
        `__**${lm.getIntl(language, 'old')}:**__ ${oldName}\n` +
        `__**${lm.getIntl(language, 'new')}:**__ ${newName}`;
