...
export async function execute(rpInstance: RustPlusInstance, args: string[],
    ...
    gInstance.generalSettings.inGameChatBotUnmuted = false;
    ...
    if (valueChanged) {
        ...
        response.push(lm.getIntl(language, 'inGameBotMessagesMuted'));
    }
    else {
        response.push(lm.getIntl(language, 'inGameBotMessagesAlreadyMuted'));
    }
