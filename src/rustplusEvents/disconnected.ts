...
export async function execute(rustplus: typeof RustPlus) {
    ...
    rustplus.info(`${lm.getIntl(Config.general.language, 'disconnectedCap')}: ${lm.getIntl(Config.general.language,
    ...
    for (const [id, timer] of Object.entries(rustplus.timers)) (timer as Timer).stop();
    ...
    if (client.activeRustplusInstances[guildId]) {
        ...
        rustplus.info(`${lm.getIntl(Config.general.language, 'reconnectingCap')}: ` +
            `${lm.getIntl(Config.general.language, 'reconnectingToServer')}`);
