...
class RustPlus extends RustPlusLib {
	constructor(guildId, serverIp, appPort, steamId, playerToken) {
		...
		this.markers =
			new Object(); /* Stores all custom markers that are created. */
		this.storageMonitors =
			new Object(); /* Contain content information of paired storage monitors. */
		this.currentSwitchTimeouts =
			new Object(); /* Stores timer ids for auto ON/OFF Smart Switch timeouts. */
		this.passedFirstSunriseOrSunset = false; /* Becomes true when first sunrise/sunset. */
		this.startTimeObject =
			new Object(); /* Stores in-game time points before first sunrise/sunset. */
		...
		this.interactionSwitches =
			[]; /* Stores the ids of smart switches that are interacted in-game. */

		this.foundSubscriptionItems =
			[]; /* Stores found vending machine items that are subscribed to */
		this.firstPollItems =
			[]; /* When a new item is added to subscription list, dont notify
		...
		this.sendTeamMessageAsync = async function (message) {
			const messageMaxLength =
				Constants.MAX_LENGTH_TEAM_MESSAGE - this.trademarkString.length;
			const strings = message.match(
				new RegExp(`.{1,${messageMaxLength}}(\\s|$)`, "g")
			);
	...
	loadRustPlusEvents() {
		const eventFiles = Fs.readdirSync(
			Path.join(__dirname, "..", "rustplusEvents")
	...
	loadMarkers() {
		...
		for (const [name, location] of Object.entries(
			instance.serverList[this.serverId].markers
		)) {
			this.markers[name] = {
				x: location.x,
				y: location.y,
	...
	build() {
		...
		this.logger = new Logger(
			Path.join(__dirname, "..", "..", `logs/${this.guildId}.log`),
			"guild"
		);
	...
	deleteThisRustplusInstance() {
		...
		if (Client.client.rustplusInstances.hasOwnProperty(this.guildId)) {
			if (
				Client.client.rustplusInstances[this.guildId].serverId === this.serverId
			) {
				this.disconnect();
	...
	async sendEvent(setting, text, firstPoll = false, image = null) {
		...
		if (!firstPoll && setting.discord) {
			await DiscordMessages.sendDiscordEventMessage(
				this.guildId,
				this.serverId,
				text,
				img
	...
	async isResponseValid(response) {
		if (response === undefined) {
			...
			this.log(
				Client.client.intlGet(null, "errorCap"),
				Client.client.intlGet(null, "responseTimeout"),
				"error"
			);
			return false;
			...
			this.log(
				Client.client.intlGet(null, "errorCap"),
				Client.client.intlGet(null, "responseIsEmpty"),
				"error"
			);
	...
	getCommandAfk() {
		...
		return string !== ""
			? `${string.slice(0, -2)}.`
			: Client.client.intlGet(this.guildId, "noOneIsAfk");
	}

	getCommandAlive(command) {
		const prefix = this.generalSettings.prefix;
		const commandAlive = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxAlive"
		)}`;
		const commandAliveEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxAlive"
		)}`;
		let name = null;
		...
		if (
			command.toLowerCase() === `${commandAlive}` ||
			command.toLowerCase() === `${commandAliveEn}`
		) {
	...
	getCommandBradley(isInfoChannel = false) {
		...
		for (const timer of Object.values(
			this.mapMarkers.bradleyAPCRespawnTimers
		)) {
			...
			if (time) {
				if (isInfoChannel) {
					...
					strings.push(
						Client.client.intlGet(this.guildId, "timeBeforeBradleyRespawns", {
							time: time,
						})
					);
				}
		...
		if (strings.length === 0) {
			if (this.mapMarkers.timeSinceBradleyAPCWasDestroyed === null) {
				...
				const secondsSince =
					(new Date() - this.mapMarkers.timeSinceBradleyAPCWasDestroyed) / 1000;
				if (isInfoChannel) {
					return Client.client.intlGet(this.guildId, "timeSinceDestroyed", {
						time: Timer.secondsToFullScale(secondsSince, "s"),
					});
				} else {
					return Client.client.intlGet(
						this.guildId,
						"timeSinceBradleyDestroyed",
						{
	...
	async getCommandCam(command) {
		const prefix = this.generalSettings.prefix;
		const commandCam = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxCam"
		)}`;
		const commandCamEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxCam"
		)}`;
		...
		const cctvs = JSON.parse(
			Fs.readFileSync(Path.join(__dirname, "..", "util/cctv.json"), "utf8")
		);

		/* airfield, bandit, dome, large, outpost, small */
		if (
			camera === Client.client.intlGet(this.guildId, "commandSyntaxList") ||
	...
	getCommandCargo(isInfoChannel = false) {
		...
		for (const [id, timer] of Object.entries(
			...
			const cargoShip = this.mapMarkers.getMarkerByTypeId(
				this.mapMarkers.types.CargoShip,
				parseInt(id)
			);
			const time = Timer.getTimeLeftOfTimer(timer);
			if (time) {
				if (isInfoChannel) {
					...
					strings.push(
						Client.client.intlGet(
							this.guildId,
							"timeBeforeCargoEntersEgressCrates",
							{
								time: time,
								location: cargoShip.location.string,
								crates: `(${cargoShip.crates.length}/3)`,
			...
			for (const id of unhandled) {
				...
				if (cargoShip.onItsWayOut) {
					...
					if (isInfoChannel) {
						...
						strings.push(
							Client.client.intlGet(this.guildId, "cargoLocatedAtCrates", {
								location: cargoShip.location.string,
								crates: `(${cargoShip.crates.length}/3)`,
							})
						);
		...
		if (strings.length === 0) {
			if (this.mapMarkers.timeSinceCargoShipWasOut === null) {
				...
				const secondsSince =
					(new Date() - this.mapMarkers.timeSinceCargoShipWasOut) / 1000;
				if (isInfoChannel) {
	...
	getCommandChinook(isInfoChannel = false) {
		...
		if (strings.length === 0) {
			if (this.mapMarkers.timeSinceCH47WasOut === null) {
				...
				const secondsSince =
					(new Date() - this.mapMarkers.timeSinceCH47WasOut) / 1000;
				if (isInfoChannel) {
					return Client.client.intlGet(this.guildId, "timeSinceLast", {
						time: Timer.secondsToFullScale(secondsSince, "s"),
					});
	...
	getCommandConnection(command) {
		const prefix = this.generalSettings.prefix;
		const commandConnection = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxConnection"
		)}`;
		const commandConnectionEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxConnection"
		)}`;
		const commandConnections = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxConnections"
		)}`;
		const commandConnectionsEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxConnections"
		)}`;
		...
		if (
			...
			if (command.toLowerCase().startsWith(`${commandConnections}`)) {
				...
				number = parseInt(
					command.slice(`${commandConnectionsEn}`.length).trim()
				);
			}
			...
			for (const player of this.team.players) {
				if (player.name.includes(name)) {
					...
					if (this.playerConnections[player.steamId].length === 0) {
						return Client.client.intlGet(
							this.guildId,
							"noRegisteredConnectionEventsUser",
							{
								user: player.name,
							}
						);
	...
	getCommandCrate(isInfoChannel = false) {
		const strings = [];
		for (const [id, timer] of Object.entries(
			this.mapMarkers.crateDespawnTimers
		)) {
			const crate = this.mapMarkers.getMarkerByTypeId(
				this.mapMarkers.types.Crate,
				parseInt(id)
			);
			const time = Timer.getTimeLeftOfTimer(timer);
			...
			if (time) {
				if (isInfoChannel) {
					...
					strings.push(
						Client.client.intlGet(this.guildId, "timeBeforeCrateDespawnsAt", {
							time: time,
							location: crate.crateType,
						})
					);
		...
		if (strings.length === 0) {
			if (this.mapMarkers.timeSinceCH47DroppedCrate === null) {
				for (const crate of this.mapMarkers.crates) {
					if (
						![
							...
							"large_oil_rig",
							"invalid",
						].includes(crate.crateType)
					) {
						if (isInfoChannel) {
							return Client.client.intlGet(this.guildId, "atLocation", {
								location:
									crate.crateType === "grid"
										? crate.location.string
										: crate.crateType,
							});
						} else {
							strings.push(
								Client.client.intlGet(this.guildId, "lockedCrateLocatedAt", {
									location:
										crate.crateType === "grid"
											? crate.location.string
											: crate.crateType,
				...
				if (strings.length === 0)
					return Client.client.intlGet(this.guildId, "noActiveLockedCrates");
			} else {
				const secondsSince =
					(new Date() - this.mapMarkers.timeSinceCH47DroppedCrate) / 1000;
				if (isInfoChannel) {
	...
	async getCommandDeath(command, callerSteamId) {
		const prefix = this.generalSettings.prefix;
		const commandDeath = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxDeath"
		)}`;
		const commandDeathEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxDeath"
		)}`;
		const commandDeaths = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxDeaths"
		)}`;
		const commandDeathsEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxDeaths"
		)}`;
		...
		if (
			...
			for (const event of this.allDeaths) {
				...
				if (event.location === null) {
					if (counter === number)
						return `${str}${Client.client.intlGet(this.guildId, "unknown")}`;
					strings.push(
						`${str}${Client.client.intlGet(this.guildId, "unknown")}`
					);
				} else {
					const distance = Math.floor(
						Map.getDistance(caller.x, caller.y, location.x, location.y)
					);
			...
			if (player.name.includes(name)) {
				...
				for (const event of this.playerDeaths[player.steamId]) {
					...
					if (event.location === null) {
						...
						const grid = location.location;
						str += Client.client.intlGet(
							this.guildId,
							"distanceDirectionGrid",
							{
								distance: distance,
								direction: direction,
								grid: grid,
		...
		if (strings.length === 0) {
			...
			if (wasOnMap == null && wasDestroyed === null) {
				return isInfoChannel
					...
					: Client.client.intlGet(
							this.guildId,
							"patrolHelicopterNotCurrentlyOnMap"
					  );
				...
				if (isInfoChannel) {
					...
					return Client.client.intlGet(
						this.guildId,
						"timeSincePatrolHelicopterWasOnMap",
						{
							time: timeSince,
						}
					);
				...
				if (isInfoChannel) {
					const timeSinceOnMap = Timer.secondsToFullScale(
						(new Date() - wasOnMap) / 1000,
						"s"
					);
					const timeSinceDestroyed = Timer.secondsToFullScale(
						(new Date() - wasDestroyed) / 1000,
						"s"
					);
					return Client.client.intlGet(
						this.guildId,
						"timeSinceLastSinceDestroyedShort",
						{
					...
					return Client.client.intlGet(
						this.guildId,
						"timeSinceLastSinceDestroyedLong",
						{
							time1: timeSinceOnMap,
							time2: timeSinceDestroyed,
						}
	...
	getCommandLarge(isInfoChannel = false) {
		const strings = [];
		for (const [id, timer] of Object.entries(
			this.mapMarkers.crateLargeOilRigTimers
		)) {
			const crate = this.mapMarkers.getMarkerByTypeId(
				this.mapMarkers.types.Crate,
				parseInt(id)
			...
			if (time) {
				if (isInfoChannel) {
					...
					strings.push(
						Client.client.intlGet(
							this.guildId,
							"timeBeforeCrateAtLargeOilRigUnlocks",
							{
								time: time,
								location: crate.location.location,
							}
		...
		if (strings.length === 0) {
			if (this.mapMarkers.timeSinceLargeOilRigWasTriggered === null) {
				...
				const secondsSince =
					(new Date() - this.mapMarkers.timeSinceLargeOilRigWasTriggered) /
					1000;
				if (isInfoChannel) {
					...
					return Client.client.intlGet(
						this.guildId,
						"timeSinceHeavyScientistsOnLarge",
						{
							time: Timer.secondsToFullScale(secondsSince),
						}
					);
	...
	async getCommandLeader(command, callerSteamId) {
		const prefix = this.generalSettings.prefix;
		const commandLeader = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxLeader"
		)}`;
		const commandLeaderEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxLeader"
		)}`;
		...
		const instance = Client.client.getInstance(this.guildId);
		if (
			!Object.keys(instance.serverListLite[this.serverId]).includes(
				this.team.leaderSteamId
			)
		) {
			let names = "";
			for (const player of this.team.players) {
		...
		if (
			command.toLowerCase() === `${commandLeader}` ||
			command.toLowerCase() === `${commandLeaderEn}`
		) {
	...
	async getCommandMarker(command, callerSteamId) {
		const prefix = this.generalSettings.prefix;
		const commandMarker = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxMarker"
		)}`;
		const commandMarkerEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxMarker"
		)}`;
		const commandMarkers = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxMarkers"
		)}`;
		const commandMarkersEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxMarkers"
		)}`;
		const commandAdd = `${Client.client.intlGet(
			this.guildId,
			"commandSyntaxAdd"
		)}`;
		const commandAddEn = `${Client.client.intlGet("en", "commandSyntaxAdd")}`;
		const commandRemove = `${Client.client.intlGet(
			this.guildId,
			"commandSyntaxRemove"
		)}`;
		const commandRemoveEn = `${Client.client.intlGet(
			"en",
			"commandSyntaxRemove"
		)}`;

		if (
			command.toLowerCase() === `${commandMarkers}` ||
			command.toLowerCase() === `${commandMarkersEn}`
		) {
			let str = "";
			for (const name in this.markers)
				str += `${name} [${this.markers[name].location}], `;

			return str !== ""
				? str.slice(0, -2)
				: Client.client.intlGet(this.guildId, "noRegisteredMarkers");
		}
			...
			case commandAdd:
				{
					if (name.startsWith(commandAdd) || name.startsWith(commandRemove))
						return null;
					if (name.startsWith(commandAddEn) || name.startsWith(commandRemoveEn))
						return null;
					if (name === "") return null;
	...
	getCommandMarket(command) {
		const instance = Client.client.getInstance(this.guildId);
		const prefix = this.generalSettings.prefix;
		const commandMarket = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxMarket"
		)}`;
		const commandMarketEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxMarket"
		)}`;
		const commandSearch = `${Client.client.intlGet(
			this.guildId,
			"commandSyntaxSearch"
		)}`;
		const commandSearchEn = `${Client.client.intlGet(
			"en",
			"commandSyntaxSearch"
		)}`;
		const commandSub = `${Client.client.intlGet(
			this.guildId,
			"commandSyntaxSubscribe"
		)}`;
		const commandSubEn = `${Client.client.intlGet(
			"en",
			"commandSyntaxSubscribe"
		)}`;
		const commandUnsub = `${Client.client.intlGet(
			this.guildId,
			"commandSyntaxUnsubscribe"
		)}`;
		const commandUnsubEn = `${Client.client.intlGet(
			"en",
			"commandSyntaxUnsubscribe"
		)}`;
		const commandList = `${Client.client.intlGet(
			this.guildId,
			"commandSyntaxList"
		)}`;
			...
			case commandSearch:
				{
					...
					for (const vendingMachine of this.mapMarkers.vendingMachines) {
						...
						for (const order of vendingMachine.sellOrders) {
							...
							if (
								order.itemId === parseInt(itemId) ||
								order.currencyId === parseInt(itemId)
							) {
								if (locations.includes(vendingMachine.location.location))
									continue;
			...
			case commandUnsub:
				{
					...
					if (instance.marketSubscriptionListItemIds.includes(itemId)) {
						instance.marketSubscriptionListItemIds =
							instance.marketSubscriptionListItemIds.filter(
								(e) => e !== itemId
							);
						Client.client.setInstance(this.guildId, instance);
	...
	getCommandNote(command) {
		const instance = Client.client.getInstance(this.guildId);
		const prefix = this.generalSettings.prefix;
		const commandNote = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxNote"
		)}`;
		const commandNoteEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxNote"
		)}`;
		const commandNotes = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxNotes"
		)}`;
		const commandNotesEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxNotes"
		)}`;
		const commandAdd = `${Client.client.intlGet(
		...
		if (
			command.toLowerCase() === `${commandNotes}` ||
			command.toLowerCase() === `${commandNotesEn}`
		) {
			...
			for (const [id, note] of Object.entries(
				instance.serverList[this.serverId].notes
			)) {
			...
			case commandAdd:
				{
					let index = 0;
					while (
						Object.keys(instance.serverList[this.serverId].notes)
							.map(Number)
							.includes(index)
					) {
						index += 1;
	...
	getCommandPlayer(command) {
		...
		const prefix = this.generalSettings.prefix;
		const commandPlayer = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxPlayer"
		)}`;
		const commandPlayerEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxPlayer"
		)}`;
		const commandPlayers = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxPlayers"
		)}`;
		const commandPlayersEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxPlayers"
		)}`;
		...
		if (
			!Object.keys(Client.client.battlemetricsOnlinePlayers).includes(
				battlemetricsId
			)
		) {
			return Client.client.intlGet(
				this.guildId,
				"couldNotFindPlayersForThisServer"
			);
		}

		let foundPlayers = [];
		if (
			command.toLowerCase() === `${commandPlayers}` ||
			command.toLowerCase() === `${commandPlayersEn}`
		) {
			foundPlayers =
				Client.client.battlemetricsOnlinePlayers[battlemetricsId].slice();
			...
			for (const player of Client.client.battlemetricsOnlinePlayers[
				battlemetricsId
			]) {
		...
		const messageMaxLength =
			Constants.MAX_LENGTH_TEAM_MESSAGE - this.trademarkString.length;
		const leftLength = `...xxx ${Client.client.intlGet(this.guildId, "more")}.`
			.length;
		...
		for (const player of foundPlayers) {
			...
			if (string.length + playerString.length + leftLength < messageMaxLength) {
				...
				string.length + playerString.length + leftLength >
				messageMaxLength
			) {
				break;
			}
	...
	getCommandPop(isInfoChannel = false) {
		if (isInfoChannel) {
			return (
				`${this.info.players}${
					this.info.isQueue() ? `(${this.info.queuedPlayers})` : ""
			...
			const queuedPlayers = this.info.isQueue()
				? ` ${Client.client.intlGet(this.guildId, "populationQueue", {
						number: this.info.queuedPlayers,
				  })}`
				: "";
	...
	async getCommandProx(command, callerSteamId) {
		const caller = this.team.getPlayer(callerSteamId);
		const prefix = this.generalSettings.prefix;
		const commandProx = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxProx"
		)}`;
		const commandProxEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxProx"
		)}`;
		...
		if (
			...
			let players = [...this.team.players].filter(
				(e) => e.steamId !== callerSteamId && e.isAlive === true
			);
			if (players.length === 0) {
			...
			for (const player of closestPlayers) {
				const distance = Math.floor(
					Map.getDistance(player.x, player.y, caller.x, caller.y)
				);
		...
		for (const player of this.team.players) {
			if (player.name.includes(memberName)) {
				const distance = Math.floor(
					Map.getDistance(caller.x, caller.y, player.x, player.y)
				);
				const direction = Map.getAngleBetweenPoints(
					caller.x,
					caller.y,
	...
	async getCommandSend(command, callerName) {
		...
		const prefix = this.generalSettings.prefix;
		const commandSend = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxSend"
		)}`;
		const commandSendEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxSend"
		)}`;
		...
		for (const player of this.team.players) {
			if (player.name.includes(name)) {
				...
				const user = await DiscordTools.getUserById(
					this.guildId,
					discordUserId
				);
				...
				const content = {
					embeds: [
						DiscordEmbeds.getUserSendEmbed(
							this.guildId,
							this.serverId,
							callerName,
							message
	...
	getCommandSmall(isInfoChannel = false) {
		const strings = [];
		for (const [id, timer] of Object.entries(
			this.mapMarkers.crateSmallOilRigTimers
		)) {
			const crate = this.mapMarkers.getMarkerByTypeId(
				this.mapMarkers.types.Crate,
				parseInt(id)
			...
			if (time) {
				if (isInfoChannel) {
					...
					strings.push(
						Client.client.intlGet(
							this.guildId,
							"timeBeforeCrateAtSmallOilRigUnlocks",
							{
								time: time,
								location: crate.location.location,
							}
		...
		if (strings.length === 0) {
			if (this.mapMarkers.timeSinceSmallOilRigWasTriggered === null) {
				...
				const secondsSince =
					(new Date() - this.mapMarkers.timeSinceSmallOilRigWasTriggered) /
					1000;
				if (isInfoChannel) {
					return Client.client.intlGet(this.guildId, "timeSinceLastEvent", {
						time: Timer.secondsToFullScale(secondsSince, "s"),
					...
					return Client.client.intlGet(
						this.guildId,
						"timeSinceHeavyScientistsOnSmall",
						{
							time: Timer.secondsToFullScale(secondsSince),
						}
					);
	...
	getCommandTime(isInfoChannel = false) {
		...
		if (isInfoChannel) {
			...
			const currentTime = Client.client.intlGet(this.guildId, "inGameTime", {
				time: time,
			});
			...
			const locString = this.time.isDay()
				? "timeTillNightfall"
				: "timeTillDaylight";
			const timeTilltransition = Client.client.intlGet(
				this.guildId,
				locString,
				{ time: timeLeft }
			);
	...
	getCommandTimer(command) {
		const prefix = this.generalSettings.prefix;
		const commandTimer = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxTimer"
		)}`;
		const commandTimerEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxTimer"
		)}`;
		const commandTimers = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxTimers"
		)}`;
		const commandTimersEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxTimers"
		)}`;
		const commandAdd = `${Client.client.intlGet(
		...
		if (
			command.toLowerCase() === `${commandTimers}` ||
			command.toLowerCase() === `${commandTimersEn}`
		) {
			...
			case commandAdd:
				{
					...
					if (message === "")
						return Client.client.intlGet(this.guildId, "missingTimerMessage");

					const timeSeconds = Timer.getSecondsFromStringTime(time);
					if (timeSeconds === null)
						return Client.client.intlGet(this.guildId, "timeFormatInvalid");
					...
					this.timers[id] = {
						timer: new Timer.timer(() => {
							this.printCommandOutput(
								Client.client.intlGet(this.guildId, "timer", {
									message: message,
								}),
								"TIMER"
							);
			...
			case commandRemove:
				{
					...
					if (isNaN(id))
						return Client.client.intlGet(this.guildId, "timerIdInvalid");
	...
	async getCommandTranslateTo(command) {
		const prefix = this.generalSettings.prefix;
		const commandTr = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxTranslateTo"
		)}`;
		const commandTrEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxTranslateTo"
		)}`;
		const commandLanguage = `${Client.client.intlGet(
			this.guildId,
			"commandSyntaxLanguage"
		)}`;
		const commandLanguageEn = `${Client.client.intlGet(
			"en",
			"commandSyntaxLanguage"
		)}`;
		...
		if (
			...
			if (
				...
				language = command
					.slice(`${commandTr} ${commandLanguage} `.length)
					.trim();
			} else {
				language = command
					.slice(`${commandTrEn} ${commandLanguageEn} `.length)
					.trim();
			}
	...
	async getCommandTranslateFromTo(command) {
		const prefix = this.generalSettings.prefix;
		const commandTrf = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxTranslateFromTo"
		)}`;
		const commandTrfEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxTranslateFromTo"
		)}`;
	...
	async getCommandTTS(command, callerName) {
		const prefix = this.generalSettings.prefix;
		const commandTTS = `${prefix}${Client.client.intlGet(
			this.guildId,
			"commandSyntaxTTS"
		)}`;
		const commandTTSEn = `${prefix}${Client.client.intlGet(
			"en",
			"commandSyntaxTTS"
		)}`;
	...
	getCommandUpkeep() {
		...
		for (const [key, value] of Object.entries(
			instance.serverList[this.serverId].storageMonitors
		)) {
			if (value.type !== "toolCupboard") continue;

			if (value.upkeep) {
				cupboardFound = true;
				const upkeepStr = Client.client
					.intlGet(this.guildId, "upkeep")
					.toLowerCase();
		...
		if (!cupboardFound)
			return Client.client.intlGet(this.guildId, "noToolCupboardWereFound");
