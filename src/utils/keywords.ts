...
export function getListOfUsedKeywords(guildId: string, serverId: string): string[] {
    ...
    if (!gInstance.serverList.hasOwnProperty(serverId)) return [];

    let list: string[] = [];
    list = [...getListOfCommandKeywords(gInstance.generalSettings.language)];

    for (const content of Object.values(gInstance.serverList[serverId].smartAlarms)) {
        list.push(content.command);
    }

    for (const content of Object.values(gInstance.serverList[serverId].smartSwitches)) {
        list.push(content.command);
    }

    for (const content of Object.values(gInstance.serverList[serverId].smartSwitchGroups)) {
        list.push(content.command);
    }
